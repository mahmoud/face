...
name: release
description: |
	Release face to PyPI. Handles version bumping (CalVer YY.MINOR.MICRO),
	tagging, pushing, and post-publish verification. Use when asked to
	"release face", "cut a release", "publish to PyPI", or "bump version".
---

# Release face

face uses CalVer: `YY.MINOR.MICRO` (e.g. `26.0.1`). The version lives in
`face/__init__.py` as a `__version__` literal string. During development it
...
3. `face/__init__.py` has a `dev` suffix on `__version__`
4. All tests pass: `tox -p auto` (or at minimum `pytest face/test/ -v`)
5. Check what is actually published on PyPI: https://pypi.org/project/face/
...
Read `__version__` from `face/__init__.py`. Strip the `dev` suffix.
...
Edit `face/__init__.py`: remove the `dev` suffix from `__version__`.
...
Add a new section at the top of `CHANGELOG.md` (below the `# face Changelog`
...
```bash
git commit -am "face version 26.0.1"
```

Use the exact format `face version X.Y.Z` for the commit message.
...
```bash
python3 -m venv /tmp/face-verify && source /tmp/face-verify/bin/activate
pip install face==26.0.1 --index-url https://pypi.org/simple/
cd /tmp  # MUST leave repo root so local face/ does not shadow the install
python -c "import face; print(face.__version__)"
# Should print: 26.0.1
deactivate && rm -rf /tmp/face-verify
```
...
- **Failed release** (tag exists locally/on GitHub but not on PyPI): PyPI is
	...
	```
	Then check `__version__` in `face/__init__.py`. If it was already bumped
